...
export default function EditAlarmScreen() {
  ...
  const loadAlarm = async () => {
    try {
      ...
      if (alarm) {
        ...
        setAdvanceMinutes(alarm.advanceMinutes || 5);
      } else {
        Alert.alert("Ошибка", "Будильник не найден");
        router.back();
      }
    } catch (error) {
      Alert.alert("Ошибка", "Не удалось загрузить будильник");
      router.back();
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert("Ошибка", "Не удалось обновить будильник");
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fce7f3" }}>
      ...
      <ScrollView style={{ flex: 1, padding: 16 }}>
        ...
        >
          <Text style={{ fontSize: 16, color: "#374151" }}>Активен</Text>
          <Switch
            value={isEnabled}
            onValueChange={setIsEnabled}
            trackColor={{ false: "#d1d5db", true: "#fbcfe8" }}
            thumbColor={isEnabled ? "#db2777" : "#f472b6"}
          />
        </View>
